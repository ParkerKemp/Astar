...
public:
		...
		const float *bottom_color)
	{
		...
		for (i = text; *i != '\0'; i++)
		{
            ...
            printf("%c dx: %f dy: %f tx1: %f ty1: %f tx2: %f ty2: %f w: %f h: %f tW: %i tH: %i\n", *i, glfont_char->dx, glfont_char->dy, glfont_char->tx1, glfont_char->ty1, glfont_char->tx2, glfont_char->ty2, width, height, header.tex_width, header.tex_height);
